...
/*jshint -W083 */
/*loopfunc: true*/
var config = require("../Link").config,
...
var MusicGraph = function (myArtists, options) {
	"use strict";
	this.api = null;;
	...
	this.getArtist = function() {
		...
		return new Promise(function(resolve, reject) {
			...
			if (! artist.externals || !artist.externals.musicgraph || !artist.externals.musicgraph.id) {
	...
	this.generateArtistPlaylist = function (artistId) {
		...
		return new Promise(function(resolve, reject) {
			...
			if (self.options.tempo) {
				query.tempo = self.options.tempo;
			}
		...
		return new Promise(function(resolve, reject) {
			...
			self.getInitialArtists()
				.then(function() {
					...
					var promises = [];
					console.log(JSON.stringify(self.similarTo, null, 4));
					_.forEach(self.similarTo, function(artist) {
						promises.push(self.generateArtistPlaylist(artist.externals.musicgraph.id));
					});
					Promise.all(promises)
						.then(function(res) {
							res = res.map(function(data) {
								var i = Math.floor((Math.random() * data.length));
								return {track: {title:data[i].title, artist: data[i].artist_name}};
							});
							self.playlist = res;
							resolve(res);
						}).catch(function(err) {
		...
		return new Promise(function(resolve, reject) {
			...
			self.getInitialArtists()
				.then(function() {
					...
					var promises = [];
					_.forEach(self.similarTo, function(artist) {
						promises.push(MusicGraph.generateArtistPlaylist(artist.externals.musicgraph.id));
					});
					Promise.all(promises)
						.then(function(res) {
							self.playlist = res;
